...
export default function MobileMenu({ open, setOpen }: MobileMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const sections = ["about", "investments", "impact", "contact"]
  ...
  const navItems = [
    { href: "#about", label: "About", icon: <Home className="h-5 w-5" /> },
          ...
          <nav className="flex-1 overflow-auto py-8 px-5">
            ...
            <div className="relative mt-12 mb-8">
              ...
              <div className="relative flex justify-center">
                <span className="bg-black px-4 text-xs uppercase tracking-wider text-white/40">ESS Platform</span>
              </div>
            </div>
            ...
            {/* <div className="flex justify-center gap-3 mb-4">
              {[Twitter, Linkedin, Github].map((Icon, idx) => (
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  className="rounded-full text-white/60 hover:text-white hover:bg-white/10 transition-all duration-300 h-9 w-9"
                >
